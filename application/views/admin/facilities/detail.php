            ...
            <div class="row">
              ...
              <div class="col-md-6">
                <table class="table table-borderless">
                  ...
                  <tr>
                    <td><strong>Dokumen BAST</strong></td>
                    <td>
                      <?php if(!empty($facility->dokumen_bast)): ?>
                        <?php
                        $file_path_bast = 'uploads/facilities/'.$facility->dokumen_bast;
                        $file_extension_bast = strtolower(pathinfo($facility->dokumen_bast, PATHINFO_EXTENSION));
                        $is_image_bast = in_array($file_extension_bast, ['jpg', 'jpeg', 'png', 'gif']);
                        ?>
                        
                        <?php if($is_image_bast): ?>
                          <div class="mb-2">
                            <img src="<?php echo base_url($file_path_bast); ?>"
                                 alt="Dokumen BAST"
                                 class="img-thumbnail"
                                 style="max-width: 200px; max-height: 150px; cursor: pointer;"
                                 onclick="window.open('<?php echo base_url($file_path_bast); ?>', '_blank')">
                          </div>
                          <a href="<?php echo base_url($file_path_bast); ?>" target="_blank" class="btn btn-sm btn-info">
                            <i class="fas fa-search-plus"></i> Lihat Gambar
                          </a>
                        <?php else: ?>
                          <a href="<?php echo base_url($file_path_bast); ?>" target="_blank" class="btn btn-sm btn-info">
                        ...
                        <a href="<?php echo base_url($file_path_bast); ?>" download="<?php echo $facility->dokumen_bast; ?>" class="btn btn-sm btn-success ml-1">
                          <i class="fas fa-download"></i> Download
                        </a>
                      <?php else: ?>
                        <span class="text-muted">Tidak ada dokumen</span>
