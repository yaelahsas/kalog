...
class Facility_Types extends CI_Controller {
    ...
    public function delete_ajax($id = null)
    {
        ...
        if ($id == null) {
            ajax_not_found_response('Facility Type ID is required');
            return;
        }
        
        try {
            if ($this->M_FacilityType->delete_facility_type($id)) {
                ajax_success_response('Jenis fasilitas berhasil dihapus!');
            } else {
                ajax_error_response('Gagal menghapus jenis fasilitas. Jenis fasilitas mungkin sedang digunakan oleh fasilitas.');
            }
        } catch (Exception $e) {
            ajax_error_response('Error deleting facility type: ' . $e->getMessage());
        }
    }
